...
블록체인은 참여자의 범위와 권한에 따라 크게 세 가지로 나뉩니다. 각 유형은 서로 다른 목적을 위해 설계되었습니다.

- **🌐 퍼블릭 블록체인 (Public Blockchain)**
    - **개념**: 인터넷처럼 누구나 익명으로 네트워크에 참여하여 거래를 생성하고 검증할 수 있는 완전히 개방된 블록체인입니다. 완전한 탈중앙화를 지향합니다.
    - **특징**: 모든 기록이 투명하게 공개되어 신뢰도가 높지만, 많은 참여자의 합의를 거쳐야 하므로 속도가 상대적으로 느립니다.
    - **대표 플랫폼**: 비트코인(Bitcoin), 이더리움(Ethereum)
- **🏢 프라이빗 블록체인 (Private Blockchain)**
    - **개념**: 단일 기관이나 기업이 중앙에서 관리하며, 사전에 허가받은 참여자만 접근할 수 있는 폐쇄형 블록체인입니다.
...
- **🤝 컨소시엄 블록체인 (Consortium Blockchain)**
    - **개념**: 여러 기관이나 기업이 연합체(컨소시엄)를 구성하여 공동으로 운영하는 블록체인입니다. 프라이빗과 퍼블릭의 중간 형태로, 허가된 기관들만 참여합니다.
    - **특징**: 참여 기관 간의 신뢰를 바탕으로 운영되므로 퍼블릭보다 효율적이면서도 프라이빗보다 탈중앙화된 구조를 가집니다. 공급망 관리나 금융 기관 간의 협업에 적합합니다.
    - **대표 플랫폼 및 프로젝트**: R3 코다(R3 Corda), 콘투어(Contour)
...
### 🧱 블록에 실제로 담기는 데이터 (비트코인 vs 이더리움)

블록에는 거래 내역과 같은 실제 데이터가 담기고, **체인**은 이 블록들의 **순서를 보장**하는 구조입니다. 이 순서를 네트워크 참여자들이 모두 동의하게 만드는 과정이 바로 **합의**이며, 그 규칙이 **합의 알고리즘**입니다.

블록체인마다 블록에 담는 데이터의 종류와 구조가 다릅니다. 가장 대표적인 비트코인과 이더리움의 예시는 다음과 같습니다.

### **1. 비트코인 (Bitcoin) 블록: 금융 거래 장부**

비트코인의 블록은 간단하고 명확한 **디지털 현금 거래 내역**을 담는 데 집중합니다.

- **거래 데이터**: 가장 핵심적인 정보로, "**누가 (From) → 누구에게 (To) → 얼마의 비트코인을 (Amount) 보냈는가**"에 대한 기록들입니다. 하나의 블록에는 수천 개의 거래가 담길 수 있습니다.
- **블록 헤더 (Block Header)**: 거래 데이터 외의 관리 정보입니다.
    - **이전 블록 해시**: 바로 앞 블록과의 연결고리입니다.
    - **머클 루트 (Merkle Root)**: 블록에 포함된 모든 거래 데이터를 요약한 하나의 해시값입니다. 거래 내역 중 하나라도 위조되면 이 값이 바뀌므로, 데이터의 무결성을 효율적으로 검증할 수 있습니다.
    - **타임스탬프 (Timestamp)**: 블록이 생성된 시간이 기록됩니다.
    - **논스 (Nonce)**: 작업증명(PoW)에서 채굴자들이 찾아내야 하는 특정 숫자 값입니다.

### **2. 이더리움 (Ethereum) 블록: 분산된 슈퍼컴퓨터의 상태 기록**

이더리움은 비트코인보다 훨씬 복잡한 데이터를 다룹니다. 단순한 거래 장부를 넘어 **'스마트 계약(Smart Contract)'이라는 프로그램 코드를 실행**할 수 있기 때문입니다.

- **거래 데이터**: 비트코인처럼 "**누가 → 누구에게 → 얼마의 이더(ETH)를 보냈는가**"라는 정보는 기본입니다.
- **스마트 계약 실행 데이터**: "**누가 → 어떤 스마트 계약을 실행했는가** (예: NFT 발행, DeFi 서비스 이용 등)"와 같은 프로그램 실행 기록이 포함됩니다. 이것이 이더리움을 '분산된 컴퓨터'라고 부르는 이유입니다.
- **상태 루트 (State Root)**: 이더리움 네트워크에 존재하는 모든 계정(사용자, 스마트 계약)의 잔액, 데이터 등의 '상태'를 요약한 해시값입니다. 블록이 하나 추가될 때마다 이 상태가 업데이트됩니다.

결론적으로, 비트코인 블록이 **'거래 기록'** 에 가깝다면, 이더리움 블록은 **'컴퓨터의 상태 변화 기록'** 에 더 가깝다고 볼 수 있습니다.
